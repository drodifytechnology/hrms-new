...
class JobApplicationController extends Controller
{
    ...
    public function jobBoardConvertData(Request $request, $id)
    {
        ...
        if (!empty($employee)) {
            ...
            $JobOnBoard->save();
            // $application_id= $JobOnBoard->application;
// $jobstage_id= JobStage::where('created_by', \Auth::user()->creatorId())->where('title', 'Hired')->pluck('id')->first();
//             $jobApplication = JobApplication::find($application_id);
//              $jobApplication->stage = $jobstage_id;
//             $jobApplication->save();
        ...
        if ($setings['new_employee'] == 1) {
            ...
            return redirect()->back()->with('success', __('Application successfully converted to employee.') . (isset($smtp_error) ? $smtp_error : '') . ((isset($result) && $result != 1) ? '<br> <span class="text-danger">' . $result . '</span>' : ''));
        }

        return redirect()->back()->with('success', __('Application successfully converted to employee.'));
        // return redirect()->back()->with('success', __('Application successfully converted to employee.') . ((isset($result) && $result != 1) ? '<br> <span class="text-danger">' . $result . '</span>' : ''));
    }
